...
- The Crossplane CLI's `render`, `beta validate`, and `beta trace` commands are the authoritative runtime/external checks. `render` executes functions via Docker by default, `beta validate` is offline schema/CEL validation, and `beta trace` shows live object relationships from a kubeconfig-backed cluster view. `render` and `trace` must remain explicit, trust-gated commands rather than default editor diagnostics. Source: https://docs.crossplane.io/latest/cli/command-reference/
...
Use fixture-backed or simulated render paths first. Use real `crossplane render` only when a fixture or user request needs proof of function execution and the workspace has explicitly enabled trusted execution. Use `crossplane beta validate` when a schema or CEL answer must be authoritative. Use `crossplane beta trace` only for live object-topology questions, after an explicit trusted-cluster/read gate. Keep Upbound `xpls` reference-only and do not let its current transport model narrow the product contract.
