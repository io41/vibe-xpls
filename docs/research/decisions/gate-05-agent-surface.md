...
Make AI agents first-class through a read-only structured JSON CLI first for terminal, CI, and repository-scanning agents, backed by the shared analyzer. Do not require agents to scrape cursor-oriented LSP methods. Editor-embedded agents with unsaved buffers need an explicit overlay model before full support can be claimed. Evaluate MCP, JSON-RPC sessions, or CLI overlay inputs after analyzer contracts, CLI response shapes, and trust boundaries stabilize. Keep first-scope render fixture-backed or behind explicit trust gates.
...
- `docs/research/lanes/03-agent-semantic-api.md` now splits terminal/CI agents from editor-embedded agents, because editor agents may need unsaved overlays and multi-file draft state that a file-only CLI cannot see.
...
- Claim editor-agent support from the file-backed CLI alone. Rejected because unsaved buffers and multi-file draft state can diverge from disk.
...
- Editor-embedded agents can receive stale analysis if the first CLI reads only disk while the user has unsaved edits or multi-file drafts.
...
- An overlay-aware JSON-RPC session or CLI overlay input proves safer and more useful than file-backed commands for the first agent workflows.
